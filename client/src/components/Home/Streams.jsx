...
const StreamCard = ({ icon, title, description, exploreLink }) => (
  <div
    className="bg-bg dark:shadow-gray-800 mx-7 sm:mx-auto rounded-lg p-6 text-center shadow-md hover:shadow-xl hover:-translate-y-2 relative overflow-hidden group transition-all duration-1000 before:absolute before:inset-0 before:rounded-lg before:border-2 before:border-transparent before:transition-all before:duration-300 hover:before:border-primary hover:before:shadow-[0_0_15px_rgba(var(--color-primary-rgb),0.5)] dark:hover:before:shadow-[0_0_15px_rgba(var(--color-primary-dark-rgb),0.5)]"
    data-aos="fade-up"
  >
    ...
    <Link
      to={exploreLink}
      className="inline-block px-6 py-2.5 rounded-full bg-[var(--color-primary)]/10 text-[var(--color-primary)] hover:bg-[var(--color-primary)] hover:text-white transition-all duration-300 relative z-10 border border-[var(--color-primary)]/20 hover:border-[var(--color-primary)] hover:shadow-lg"
    >
      Explore
    </Link>
  </div>
